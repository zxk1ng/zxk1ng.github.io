...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | zxk1ng🍓")[0];
}
...
$.ajax({
  ...
  data: {
    key: 'NFSBZ-JQ4CI-PATGH-UA2YI-AM3PO-7MFVL',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
      ...
      owo_body.onmouseover = (e) => {
        if (flag && e.target.tagName == 'IMG') {
          ...
          owo_time = setTimeout(() => {
            ...
            // 在盒子中插入图片
            div.innerHTML = `<img src="/js/fomal.jsundefined" >`
          }, 300);
        }
      };
      // 鼠标移出隐藏盒子
...
function createtime1() {
  var grt = new Date("03/04/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
  ...
  var ascll = [
    ...
    `
                ...
                __    ____                
___________  __|  | _/_   | ____    ____  
\___   /\  \/  /  |/ /|   |/    \  / ___\ 
 /    /  >    <|    < |   |   |  \/ /_/  >
/_____ \/__/\_ \__|_ \|___|___|  /\___  / 
      \/      \/    \/         \//_____/  
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| zxk1ng🍓") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('zxk1ng🍓的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("03/04/2023 00:00:00"); // 旅行者1号开始计算的时间
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ...
      : `<img class='boardsign' src='https://lskypro.acozycotage.net/Fomalhaut/badge/F小屋-下班休息啦.svg' title='休息了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
